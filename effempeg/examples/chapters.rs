...
fn main() {
    ...
    match effempeg::format::input(&env::args().nth(1).expect("missing input file name")) {
        Ok(ictx) => {
            ...
            let mut octx =
                effempeg::format::output(&"test.mkv".to_owned()).expect("Couldn't open test file");
            ...
            for chapter in ictx.chapters() {
                ...
                match octx.add_chapter(
                    ...
                    chapter
                        .time_base()
                        .expect("Input chapter is missing a time base"),
                    chapter.start(),
                    chapter.end(),
                    &title,
                ) {
