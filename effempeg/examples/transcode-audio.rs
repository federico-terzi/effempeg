...
use effempeg::{
    channel_layout::ChannelLayout, codec, filter, format, frame, media, rescale, Rescale,
};
...
fn filter(
...
) -> Result<filter::Graph, effempeg::Error> {
    ...
    if let Some(codec) = encoder.codec() {
        if !codec
            .capabilities()
            .contains(effempeg::codec::capabilities::Capabilities::VARIABLE_FRAME_SIZE)
        {
            filter
                .get("out")
                .unwrap()
...
struct Transcoder {
    ...
    decoder: codec::decoder::Audio,
    encoder: codec::encoder::Audio,
    in_time_base: effempeg::Rational,
    out_time_base: effempeg::Rational,
}
...
fn transcoder<P: AsRef<Path>>(
    ...
    path: &P,
    filter_spec: &str,
) -> Result<Transcoder, effempeg::Error> {
    let input = ictx
        .streams()
        .best(media::Type::Audio)
        .expect("could not find best audio stream");
    let mut decoder = input.decoder()?.audio()?;
    let codec = effempeg::encoder::find(octx.format().codec(path, media::Type::Audio))
        .expect("failed to find encoder")
        .audio()?;
    let global = octx
        .format()
        .flags()
        .contains(effempeg::format::flag::Flags::GLOBAL_HEADER);
    ...
    if global {
        encoder.set_flags(effempeg::codec::flag::Flags::GLOBAL_HEADER);
    }
...
impl Transcoder {
    fn send_frame_to_encoder(&mut self, frame: &effempeg::Frame) {
    ...
    fn receive_and_process_encoded_packets(&mut self, octx: &mut format::context::Output) {
        let mut encoded = effempeg::Packet::empty();
    ...
    fn add_frame_to_filter(&mut self, frame: &effempeg::Frame) {
    ...
    fn send_packet_to_decoder(&mut self, packet: &effempeg::Packet) {
