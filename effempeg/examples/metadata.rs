...
fn main() {
    ...
    match effempeg::format::io::input(file) {
        Ok(context) => {
            for (k, v) in context.metadata().iter() {
                println!("{}: {}", k, v);
            }

            if let Some(stream) = context.streams().best(effempeg::media::Type::Video) {
                println!("Best video stream index: {}", stream.index());
            }

            if let Some(stream) = context.streams().best(effempeg::media::Type::Audio) {
                println!("Best audio stream index: {}", stream.index());
            }

            if let Some(stream) = context.streams().best(effempeg::media::Type::Subtitle) {
            ...
            println!(
                ...
                context
                    .duration()
                    .map(|d| d as f64 / f64::from(effempeg::ffi::AV_TIME_BASE))
            );
            ...
            for stream in context.streams() {
                ...
                if codec_par.medium() == effempeg::media::Type::Video {
                ...
                } else if codec_par.medium() == effempeg::media::Type::Audio {
