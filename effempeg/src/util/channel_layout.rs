...
fn alloc_custom_channels(channels: &[CustomChannel]) -> *mut AVChannelCustom {
    unsafe {
        let map = av_malloc_array(channels.len(), mem::size_of::<AVChannelCustom>())
            .cast::<AVChannelCustom>();
...
impl ChannelLayout {
    ...
    fn contains_avchannel(&self, channel: AVChannel) -> Option<bool> {
        match self.0.order {
            AVChannelOrder::AV_CHANNEL_ORDER_NATIVE => unsafe {
                Some(self.0.u.mask & (1 << channel.0 as u64) != 0)
            },
    ...
    pub fn contains_all(&self, layout: &ChannelLayout) -> Option<bool> {
        match (self.0.order, layout.0.order) {
            ...
            (AVChannelOrder::AV_CHANNEL_ORDER_CUSTOM, AVChannelOrder::AV_CHANNEL_ORDER_NATIVE) => {
                None
            }
            ...
            // no information about channels available
            (AVChannelOrder::AV_CHANNEL_ORDER_UNSPEC, _)
            | (_, AVChannelOrder::AV_CHANNEL_ORDER_UNSPEC) => None,
            (AVChannelOrder::AV_CHANNEL_ORDER_AMBISONIC, _)
            | (_, AVChannelOrder::AV_CHANNEL_ORDER_AMBISONIC) => None,

            (self_order, layout_order) => {
                panic!("invalid channel orders: {self_order:?}, {layout_order:?}")
            }
        }
    }
    ...
    pub fn describe(&self) -> Result<String, Error> {
        ...
        ) -> Result<Result<String, usize>, Error> {
            unsafe {
                let bytes_needed = match av_channel_layout_describe(
                    layout.as_ptr(),
        ...
        match describe_into(&mut buf[..], self)? {
            ...
            Err(needed) => {
                ...
                Ok(describe_into(&mut buf[..], self)?
                    .expect("allocated buffer should have been big enough"))
            }
        }
    }
    ...
    pub fn native_order_bits(&self) -> Option<u64> {
        (self.0.order == AVChannelOrder::AV_CHANNEL_ORDER_NATIVE)
            .then_some(unsafe { self.0.u.mask })
    }

    unsafe fn custom_channels_unchecked(&self) -> &[CustomChannel] {
        slice::from_raw_parts(
            self.0.u.map.cast::<CustomChannel>(),
            self.0.nb_channels.max(0) as usize,
        )
    }
...
impl ChannelLayout {
    ...
    pub const HEXAGONAL: ChannelLayout =
        Self::_5POINT0_BACK.with_channels_native(&[Channel::BackCenter]);
    pub const MONO: ChannelLayout = Self::native(&[Channel::FrontCenter]);
    pub const OCTAGONAL: ChannelLayout = Self::_5POINT0.with_channels_native(&[
        Channel::BackLeft,
        Channel::BackCenter,
        Channel::BackRight,
    ]);
    pub const QUAD: ChannelLayout =
        Self::STEREO.with_channels_native(&[Channel::BackLeft, Channel::BackRight]);
    pub const STEREO: ChannelLayout = Self::native(&[Channel::FrontLeft, Channel::FrontRight]);
    pub const STEREO_DOWNMIX: ChannelLayout =
        Self::native(&[Channel::StereoLeft, Channel::StereoRight]);
    ...
    pub const _2_2: ChannelLayout =
        Self::STEREO.with_channels_native(&[Channel::SideLeft, Channel::SideRight]);
    pub const _3POINT1: ChannelLayout =
        Self::SURROUND.with_channels_native(&[Channel::LowFrequency]);
    pub const _4POINT0: ChannelLayout = Self::SURROUND.with_channels_native(&[Channel::BackCenter]);
    pub const _4POINT1: ChannelLayout =
        Self::_4POINT0.with_channels_native(&[Channel::LowFrequency]);
    pub const _5POINT0: ChannelLayout =
        Self::SURROUND.with_channels_native(&[Channel::SideLeft, Channel::SideRight]);
    ...
    pub const _5POINT1: ChannelLayout =
        Self::_5POINT0.with_channels_native(&[Channel::LowFrequency]);
    pub const _5POINT1_BACK: ChannelLayout =
        Self::_5POINT0_BACK.with_channels_native(&[Channel::LowFrequency]);
    ...
    pub const _6POINT1_BACK: ChannelLayout =
        Self::_5POINT1_BACK.with_channels_native(&[Channel::BackCenter]);
    pub const _6POINT1_FRONT: ChannelLayout =
        Self::_6POINT0_FRONT.with_channels_native(&[Channel::LowFrequency]);
    pub const _7POINT0: ChannelLayout =
        Self::_5POINT0.with_channels_native(&[Channel::BackLeft, Channel::BackRight]);
    pub const _7POINT0_FRONT: ChannelLayout = Self::_5POINT0
        .with_channels_native(&[Channel::FrontLeftOfCenter, Channel::FrontRightOfCenter]);
    pub const _7POINT1: ChannelLayout =
        Self::_5POINT1.with_channels_native(&[Channel::BackLeft, Channel::BackRight]);
    ...
    pub const _7POINT1_WIDE: ChannelLayout = Self::_5POINT1
        .with_channels_native(&[Channel::FrontLeftOfCenter, Channel::FrontRightOfCenter]);
    pub const _7POINT1_WIDE_BACK: ChannelLayout = Self::_5POINT1_BACK
        .with_channels_native(&[Channel::FrontLeftOfCenter, Channel::FrontRightOfCenter]);
}
...
mod serde {
    ...
    mod test {
        ...
        #[test]
        fn serde() {
            round_trip_debug(ChannelLayout::native(&[
                Channel::StereoRight,
